...
$a = ["iewil","bitsclaim","1.0"];
$reg = "https://bit.ly/3smrhzI";
...
faucet:
while(true){auto();$api = api();$time = $api["seconds"];$msg = $api["message"];$energy=explode('<span>',explode('<p class="amount">',auto())[1])[0];if($time){tmr($time);}if($msg=="no_autoclaims"){echo col('insufficient BITS Balance','m')."\n";goto menu;}api();Credit();echo col("Energy ~> ",'h').col($energy,'k')."\n";line();}
